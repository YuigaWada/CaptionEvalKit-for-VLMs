...
class BERTEncoder(Encoder):
    ...
    def __init__(
        ...
        self._max_pos = self._usable_max_positions()
    ...
    def _usable_max_positions(self) -> int:
        max_positions = self.model.config.max_position_embeddings
        if getattr(self.model.config, "model_type", "") == "roberta":
            # RoBERTa offsets position ids by padding_idx + 1, so 514 embeddings
            # only allow 512 actual tokens.
            return max_positions - self.tokenizer.padding_index - 1
